...
**Power / sample size (resolved by embedding in SARA; simulation run 02 Jul 2026).** All ~4,000 SARA respondents pass through the module (less consent declines and attention-check screen-outs), giving roughly **~390–430 per cell**. The pre-launch simulation (`power_sim_h2.R`, 4,000 reps, α=.05, direction main effect d=0.30, support ~ N(μ,1)): power for the H2 direction × route interaction in the one-sided cells is **0.83 at δ=0.15 SD and 0.97 at δ=0.20 SD even at a conservative 360/cell** (0.46 at δ=0.10). So H2 is adequately powered for interaction effects of ~0.15 SD and above. The binding constraint is the *exploratory mediation* (single-item mediators attenuate a and b paths), which no realistic n fully rescues — report those as effect estimates with CIs, not hypothesis tests.
