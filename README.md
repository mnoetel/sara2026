...
- **`survey/sara_usa.md`** — the instrument: every item, response scale, page order,
  ...
  and the review table. Being Markdown means it stays readable and reviewable in a
  **GitHub pull request** — reviewers comment on the diff, not a code editor.
...
survey/            oTree app — reads sara_usa.md, builds the Player model + pages
  sara_usa.md      THE instrument (YAML inside a fenced block; reviewed via GitHub PRs)
...
` ```yaml ` block, so a change to it reads as a clean diff in a **pull request** —
reviewers comment inline on GitHub, no separate copy to keep in sync. (We used to
round-trip through HackMD; that always drifted from the repo, so the repo is now the
single source of truth and review happens on the PR.)
...
- **Structural edits** (rewording an item, changing options, reordering) must stay
  ...
  comfortable with that, open an issue or leave a PR comment and let an editor apply it.
- Branch, edit `survey/sara_usa.md`, run `make -C render` to confirm it still parses and
  rebuilds the review table (and refreshes the protocol's auto-blocks), then open a PR.
- `spec_loader.py` is what extracts the fenced block; both the oTree app and the
  ...
  file format.
- The **protocol doc** (`SARA USA — Survey Protocol v10.md`) is prose, but where it shows
  actual survey items (the Method 1/3/4 worked examples and the superintelligence module,
  as collapsible `<details>` blocks) that content sits inside `<!-- BEGIN:auto:… -->`
  markers generated by `render/sync_protocol.py` — page-based, so adding an item to a page
  flows it into the briefing. After editing the instrument, run `make -C render` to refresh
  those blocks; `make -C render check-protocol` fails on drift.
