# SARA USA 2026 — Survey Protocol & Instrument (v7, markdown)

**Michael Noetel (UQ) & Alor Sahoo (MIT). Version date: 17 June 2026.**

This is the editable markdown master. It supersedes the v2–v5 Word docs and v6 (all archived in this folder) and folds in the live prototyping edits: the environment experiment moved first, matched-language frame item, two-orders-of-magnitude scope rows, a log willingness-to-pay scale, and the DCE-based quantitative core. **v7 changes:** the mitigations and priority-risks batteries moved to the Dumpster; the attention/quality check moved next to the Module 3 equivalence items; an FAA-style policy item added (Anthropic's proposal); a dangerous-activity sanity anchor added; and demographics aligned to US Census / ACS categories. The clickable prototype `SARA USA — full survey prototype v2.html` mirrors this document; the DCE design and analysis live in `sara_dce_design.R`.
...
- **m3_sanity (dangerous-activity anchor).** Compared with a high-risk voluntary activity [randomise: bungee jumping ~1 in 500,000 per jump; BASE jumping ~1 in 2,300; climbing Everest ~1 in 100], how safe should AI be? *A sanity check at the dangerous end (1–10% death) opposite aviation's ~1 in 10,000,000: almost everyone should say "much safer", so anyone who doesn't is flagged, and it tests whether people can place AI across the full risk range. Death rates approximate; verify before fielding.*
- **m3_attention (quality check).** Embedded here, next to the equivalence items, because it mimics the comparative-standard format ("select 'Somewhat less strict than nuclear power'"). One of a bidirectional pair. *Moved out of Module 7 so it blends in with the items it imitates.*
...
### Module 6 — Policy (Cluster A)
*(The mitigations and priority-risks batteries were moved to the Dumpster, §6.)*
- **m6_certify.** Who should certify AI as safe before deployment (companies / government / third-party / none).
- **m6_faa (FAA model).** Support/oppose regulating frontier AI like the FAA regulates aircraft: mandatory independent safety testing before release, with the government able to block or reverse an unsafe release. *This is Anthropic's stated proposal (the cars/airplanes/drugs analogy); it goes beyond the June 2026 White House EO, which is security-focused and explicitly voluntary (disavows mandatory licensing/pre-clearance). Participant sees a neutral "some have proposed", not a branded position. This elevates the single most consequential mitigation now that the battery is retired.*
- **m6_fedstate.** Federal pre-emption vs a state patchwork.
- **m6_killswitch.** Mandatory emergency shutdown (all / high-risk only / voluntary / unsure).
- **m6_liability.** Strict / negligence–duty-of-care / broke-a-rule / none-if-compliant / unsure.

### Module 7 — Validation
Within-person consistency check; **superintelligence statement [randomise Stem A campaign wording vs Stem B permission framing → wording effect]**; authenticity check (free-text, no AI tools); the counter-message arm (accel / safety / none) precedes the tolerance block.

### Module 9 — Individual differences & demographics
CRT (Benjamin Reid's items, e.g. the pig problem); Berlin-style numeracy / risk literacy; 7-point political orientation (Jost 2006); AI familiarity (frequency of use; Gillespie et al. 2025). Covariates moderate, never exclude.

Demographics for MRP weighting use **US Census / ACS categories** so they post-stratify to population controls (verify against the exact ACS pull before fielding):
- **Age:** 18–24 / 25–34 / 35–44 / 45–54 / 55–64 / 65–74 / 75+ (ACS B01001, collapsed).
- **Gender:** Male / Female / In another way–non-binary / prefer not to say. Weighting uses the ACS sex controls (Male/Female); the extra category is kept descriptively and allocated, not dropped.
- **Education:** less than high school / high-school grad / some college, no degree / associate / bachelor's / graduate or professional (ACS B15003, collapsed).
- **Income:** Census/ACS household brackets, <$15k up to $200k+ (B19001).
- **State:** all 50 + DC (full dropdown). No state map unless the per-state effective sample clears the pre-registered threshold.
...
| "I'd pay whatever it takes" WTP option | Strategic and unbounded; breaks the scale (Claude's call when fixing the WTP scale, not requested — flag if you want it back) |
| Mitigations battery ("I'd trust AI more if…") | Removed from the live instrument (MN, 17 Jun 2026). The FAA-style item in Module 6 carries the single most important mitigation; the rest sit here, recoverable. |
...
- Supersedes v6 (markdown) and the v5/v4.1/v4/v3/v2 Word docs and the archived Nov-2025 draft (all retained in this folder).
- The FAA-model item and cars/airplanes/drugs framing follow Anthropic's public proposal; contextual EO is the June 2, 2026 White House order *Promoting Advanced AI Innovation and Security* (security-focused, voluntary).
