# SARA USA 2026: Plan and Survey Protocol (v8)
...
|---|---|
| §2 | Why we need to triangulate: it's hard to ask "what risk is tolerable?" |
| §3 | Four ways we triangulate risk tolerance, each with its literature, strengths, limitations, our response, and a worked example item. |
| §4 | Moderators to explore. |
| §5 | Estimating the *population* view using MRP. |
| §6 | Testing how opinion moves with framing. |
| §7 | Secondary modules not about risk tolerance: the superintelligence ban, and data centres. |
| §8 | Criticisms and tentative responses: one table for what we have resolved, one for what we have not. |
| §9 | Open questions we still need to settle. |
| Appendices | Full fielding order (A), DCE design and analysis (B), the dumpster (C), provenance and sources (D). |
...
The four methods escalate from "judge someone else's number" to "state your own." Each subsection gives the approach in plain English, the literature behind it, its strengths and limitations, how we respond to those limitations, and a worked example with the real response scale.

### 3.1 Method 1: Judge a named expert's number
...
### 3.2 Method 2: Anchor to the safety we already accept
...
### 3.3 Method 3: Reveal preferences through choices (the discrete choice experiment)
...
### 3.4 Method 4: Ask directly, then test whether the answer is real
...
- **Need for cognition.** Requested by Muskan. Not yet in the v7 instrument. Proposed: the 18-item efficient NCS (Cacioppo, Petty & Kao 1984) or a validated short form.[^nfc] **Open item: confirm inclusion and which form (see §9).**
...
**Convergence go/no-go (pre-registered).** A single quantitative public number is reported only if the DCE (§3.3) and the direct scope instrument (§3.4) agree within about one order of magnitude in an overlap subsample. If they diverge, we report a bound plus a qualitative finding. The expert-to-public multiplier, if reported at all, is computed only on a pass, as a distribution with credible intervals, conditioned on a stated benefit scenario, with uncertainty propagated on both sides. We never manufacture an "N times safer" headline from the §3.2 back-out.
...
|---|---|---|---|
| 1 | Oscar Delaney (+Barnett) | Don't trust public probabilities; relative comparisons are more useful | Direct elicitation demoted to a validation cross-check; named-expert recognition (§3.1) and relative-standard anchoring (§3.2) are primary |
| 2 | Peter Barnett | Scope insensitivity (treat 1/100k ≈ 1/10M) | **Measured, not designed away:** within-person probability/severity, scope-slope × risk-literacy interaction, rows two OOMs apart, order randomised + between-subjects cross-check (§3.4) |
...
| 7 | Peter Barnett | Depends what you call "experts" | Every source named and randomised (LeCun, Altman, Musk, Amodei; superforecasters; XPT); reported by anchor (§3.1) |
...
| 10 | Barnett / Delaney | Give people a neutral intuition for risk levels by comparing to other fields | Relative-standards method (§3.2) plus the dangerous-activity sanity anchor |
...
| 17 | Gradient | Figure 11 category error ("systems should be stricter") | Split into stricter *standards* (§3.2a) and safer *systems* (§3.2b) |
...
8. **Verification before fielding.** (a) Every named-expert figure in §3.1 needs an exact citable quote. (b) The dangerous-activity death rates in §3.2d need checking. (c) Confirm reviewer names: the project documents record **Oscar Delaney** (not "Mollonez"); confirm this is the person Mike meant.
...
5. **Module 2: Judge a named expert's number** (§3.1): m2_tol_anchor; m2_pace (tolerability of the current pace of disruption); m2_frame_applicable.
6. **Module 3: Anchor to accepted safety** (§3.2): 3a-i standards; 3a-ii safety-in-practice; m3_sanity dangerous-activity anchor; m3_attention (bidirectional quality check embedded here).
7. **Module 4: Quantitative core** (§3.3, §3.4): the DCE; m4b_reasonable; m4c scope (within person).
8. **Module 5: Costed tradeoffs:** m5_wtp (log scale); m5b_delay_uncond and m5b_delay_cond (reported separately); m5_race (conditional slowdown).
9. **Module 6: Policy (Cluster A):** m6_certify; **m6_faa** (FAA-style mandatory independent pre-release safety testing with government power to block; Anthropic's stated proposal, presented neutrally as "some have proposed"); m6_fedstate (federal pre-emption vs state patchwork); m6_killswitch; m6_liability.
10. **Module 7: Validation:** within-person consistency; superintelligence-statement wording experiment (§6); free-text authenticity check (no AI tools); the counter-message arm precedes the tolerance block.
11. **Module 9: Individual differences and demographics** (§4).
...
- Supersedes v6/v7 (markdown) and the v5/v4.1/v4/v3/v2 Word docs and the archived Nov-2025 draft (all retained in this folder).
...
[^nfc]: Need-for-cognition: original John T. Cacioppo & Richard E. Petty, "The Need for Cognition," *J. Personality and Social Psychology* 42:116–131 (1982); efficient 18-item form, Cacioppo, Petty & Kao, "The Efficient Assessment of Need for Cognition," *J. Personality Assessment* 48:306–307 (1984). Confirm choice of form with Muskan.
